#!/usr/bin/env node
/**
 * Build Rise SVG launcher (Arctic srcdoc method) for example/index.html
 */
...
const BRANCH = "main";
const SUBPATH = "static/arctic-static-main/example";

function origins() {
	const base = `gh/${GITHUB}@${BRANCH}/${SUBPATH}`;
	return [
...
console.log(`\nCDN URL:`);
console.log(`  https://cdn.jsdelivr.net/${SUBPATH.replace("static/", `gh/${GITHUB}@${BRANCH}/static/`).replace("gh/", "gh/")}/embed.svg`);
console.log(`  https://cdn.jsdelivr.net/gh/${GITHUB}@${BRANCH}/static/arctic-static-main/example/embed.svg`);
